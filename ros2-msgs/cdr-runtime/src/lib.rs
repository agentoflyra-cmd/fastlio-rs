...
unsafe fn assume_init_vec<T>(items: Vec<MaybeUninit<T>>) -> Vec<T> {
    let mut items = ManuallyDrop::new(items);

    unsafe {
        Vec::from_raw_parts(
            items.as_mut_ptr().cast::<T>(),
            items.len(),
            items.capacity(),
        )
    }
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InvalidRepresentation(value) => {
                write!(
                    f,
                    "CdrEncoding::parse: invalid representation value {value:#06x}"
                )
            }
            ...
            Self::InvalidBool(value) => {
                write!(
                    f,
                    "CdrDecoder::read_bool: invalid boolean discriminant {value}"
                )
            }
...
impl<'a, T: PrimitiveValue> PrimitiveSeq<'a, T> {
        ...
        T: Copy,
    {
        ...
        assert_ne!(item_size, 0, "zero-sized types are not supported");
        assert_eq!(
            byte_len,
            len.checked_mul(item_size).expect("byte length overflow"),
            "source byte length does not match element count"
        );

        let mut items: Vec<MaybeUninit<T>> = Vec::with_capacity(len);
        items.resize_with(len, MaybeUninit::uninit);

        let dst_bytes =
            unsafe { slice::from_raw_parts_mut(items.as_mut_ptr().cast::<u8>(), byte_len) };

        unsafe {
            copy_bytes_into_simd(self.bytes.as_ptr(), dst_bytes.as_mut_ptr(), byte_len);
        }

        if !native_endianness_matches(self.endianness) {
            swap_endian_in_place(dst_bytes, item_size);
        }

        unsafe { assume_init_vec(items) }
    }
}
...
impl<'a, T: PrimitiveValue, const N: usize> PrimitiveArray<'a, T, N> {
    ...
    pub fn to_array(&self) -> [T; N] {
        ...
        let mut items: [MaybeUninit<T>; N] = std::array::from_fn(|_| MaybeUninit::uninit());
        unsafe {
        ...
        unsafe { items.map(|item| item.assume_init()) }
    }
}
...
impl<'a> CdrDecoder<'a> {
    ...
    fn read_swapped_primitive_seq<T: Copy>(&mut self) -> CdrResult<Vec<T>> {
        ...
        let items = unsafe { assume_init_vec(items) };
        Ok(items)
    }
    ...
    fn read_native_primitive_array<T: Copy, const N: usize>(&mut self) -> CdrResult<[T; N]> {
        ...
        Ok(unsafe { items.map(|item| item.assume_init()) })
    }
    ...
    fn read_swapped_primitive_array<T: Copy, const N: usize>(&mut self) -> CdrResult<[T; N]> {
        ...
        let item_bytes =
            unsafe { std::slice::from_raw_parts_mut(items.as_mut_ptr().cast::<u8>(), byte_len) };

        unsafe {
            copy_bytes_into_simd(bytes.as_ptr(), item_bytes.as_mut_ptr(), byte_len);
        }

        swap_endian_in_place(item_bytes, size_of::<T>());
    ...
    pub fn read_primitive_seq_borrowed<T: PrimitiveValue>(
        &mut self,
    ) -> CdrResult<PrimitiveSeq<'a, T>> {
        let length = self.read_u32()? as usize;
        self.align_to(size_of::<T>());
        let byte_len = length
            .checked_mul(size_of::<T>())
...
impl CdrEncoder {
    ...
    fn write_native_primitive_slice<T: Copy>(
        &mut self,
        value: &[T],
        write_len: bool,
    ) -> CdrResult<()> {
    ...
    fn write_swapped_primitive_slice<T: Copy>(
        ...
        swap_endian_in_place(
            &mut self.data_raw[old_len..old_len + byte_len],
            size_of::<T>(),
        );
        Ok(())
    }
...
mod test {
    use super::{
        CdrDecoder, CdrEncoder, CdrEncoding, CdrRepresentation, EncodeCdr, Endianness, WChar16,
        WChar32, decode_from_bytes, encode_to_vec,
    };
    ...
    fn read_wstring_trims_utf16_terminator() {
        let bytes = little_endian_message(&[0x02, 0x00, 0x00, 0x00, 0x41, 0x00, 0x00, 0x00]);
        let mut decoder = CdrDecoder::new(&bytes).expect("decoder should be created");
    ...
    fn read_wstring_rejects_invalid_utf16() {
        let bytes = little_endian_message(&[0x02, 0x00, 0x00, 0x00, 0x00, 0xd8, 0x00, 0x00]);
        let mut decoder = CdrDecoder::new(&bytes).expect("decoder should be created");
    ...
    fn read_octet_seq_reads_owned_bytes() {
        let bytes = little_endian_message(&[0x03, 0x00, 0x00, 0x00, 0xaa, 0xbb, 0xcc]);
        let mut decoder = CdrDecoder::new(&bytes).expect("decoder should be created");
        assert_eq!(
            decoder
                .read_octet_seq()
                .expect("octet sequence should decode"),
            vec![0xaa, 0xbb, 0xcc]
        );
    }
    ...
    fn read_seq_decodes_primitive_items() {
        let bytes = little_endian_message(&[0x03, 0x00, 0x00, 0x00, 0x11, 0x22, 0x33]);
        let mut decoder = CdrDecoder::new(&bytes).expect("decoder should be created");
        assert_eq!(
    ...
    fn read_array_decodes_primitive_items() {
        ...
        let mut decoder = CdrDecoder::new(&bytes).expect("decoder should be created");
        assert_eq!(
            decoder.read_array::<u16, 2>().expect("array should decode"),
            [0x1234, 0x5678]
        );
    }
    ...
    fn encode_round_trips_primitives() {
        ...
        assert_eq!(
            decode_from_bytes::<u32>(&bytes).expect("u32 should decode"),
            42
        );

        let bytes = encode_to_vec(&-7i16).expect("i16 should encode");
        assert_eq!(
            decode_from_bytes::<i16>(&bytes).expect("i16 should decode"),
            -7
        );

        let bytes = encode_to_vec(&3.5f32).expect("f32 should encode");
        assert_eq!(
            decode_from_bytes::<f32>(&bytes).expect("f32 should decode"),
            3.5
        );
    }
    ...
    fn write_byte_array_encodes_fixed_bytes() {
        ...
        assert_eq!(
            encoder.data_raw,
            little_endian_message(&[0x2a, 0x2b, 0x2c, 0x2d])
        );
    }
}
