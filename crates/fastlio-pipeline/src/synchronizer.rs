...
impl MeasurementSynchronizer {
    ...
    fn try_build_group(&mut self) -> Result<Option<MeasureGroup>> {
        ...
        let imu: Vec<ImuSample> = self.imu_queue.drain(..end_idx).collect();
...
mod test {
    ...
    fn lidar_waits_when_imu_does_not_cover_begin() {
        ...
        sync.pend_imu(imu(1.0)).unwrap();
        sync.pend_imu(imu(2.0)).unwrap();
        let group = sync.pend_lidar(lidar(0.0, 1.0)).unwrap().unwrap();
        assert_eq!(group.lidar.base_timestamp_sec, 0.0);
        assert_eq!(group.imu.last().unwrap().time_stamp_sec, 1.0);
    }
    ...
    fn boundary_imu_after_scan_end_included_in_group() {
        ...
        // IMU at 2.1 must remain in queue; the pipeline extrapolates to scan end.
        assert_eq!(sync.imu_queue.len(), 1);
        assert_eq!(sync.imu_queue[0].time_stamp_sec, 2.1);
    }
    ...
    #[test]
    fn adjacent_lidar_frames_package_by_end_time() {
        let mut sync = MeasurementSynchronizer::new();
